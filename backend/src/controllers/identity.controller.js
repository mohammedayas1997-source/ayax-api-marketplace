...
/* ======================================================
   3. BVN VERIFICATION (STRICT ABJIKTECH BASE64 FORMAT)
...
exports.verifyBvn = async (req, res) => {
  ...
  try {
    const user = req.user || req.apiKeyUser;
    const { bvn, bvnNumber, searchValue, slipType = "Standard Slip", reference } = req.body;
    ...
    const cost = 140;
    chargedCost = cost;
    const txRef = reference || `MKTP_BVN_${Date.now()}`;
    ...
    // Ciro asalin pdf_base64 da Abjiktech ya aiko
    const pdfBase64 =
      result?.pdf_base64 ||
      result?.data?.pdf_base64 ||
      result?.raw?.pdf_base64 ||
      null;
    ...
    if (!isSuccess || !pdfBase64) {
      const failureReason = result?.message || "BVN record could not be retrieved by gateway.";
      await refundTransaction({
        userId: chargedUserId,
        cost: chargedCost,
        transactionId: chargedTransaction.id,
    ...
    await prisma.transaction.update({
      ...
      data: {
        status: "SUCCESSFUL",
        description: `BVN Slip Generated Successfully for [${cleanBvn}]`,
      },
    });

    // Mayar da pdf_base64 ga Ayax Data Express
    return res.status(200).json({
      status: "success",
      success: true,
      message: "BVN verified and slip generated successfully.",
      pdf_base64: pdfBase64,
      userData: result?.user_data || result?.data?.user_data || null,
      data: {
        reference: txRef,
        bvn: cleanBvn,
        slipType,

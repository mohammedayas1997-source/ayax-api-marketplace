...
const serializePricing = (item) => ({
  ...
  dataType: item.metadata?.dataType || null,
  dataSize: item.metadata?.dataSize || null,
  validity: item.metadata?.validity || null,
...
exports.getPublicPricing = async (req, res) => {
  try {
    ...
    const groupedPricing = pricing.reduce((groups, item) => {
      const cat = item.category.toUpperCase();
      if (!groups[cat]) {
        groups[cat] = [];
      }
      groups[cat].push(serializePricing(item));
      return groups;
    }, {});
...
exports.getAdminPricing = async (req, res) => {
  try {
    const { category, tier, enabled, search } = req.query;
...
/* ======================================================
   ADMIN: CREATE PLAN (WITH DATA TYPE & VALIDITY)
...
exports.createPricing = async (req, res) => {
  try {
    ...
    const cleanCode = normalizeUppercase(serviceCode);
    const cleanName = normalizeText(serviceName);
    const cleanCategory = normalizeUppercase(category);
    const normalizedTier = normalizeUppercase(tier);
    const parsedCostPrice = parseAmount(costPrice);
    const parsedSellingPrice = parseAmount(sellingPrice);

    if (!cleanCode || !cleanName || !cleanCategory) {
      return res.status(400).json({
        success: false,
        message: "Service code, name, and category are required.",
      });
    }
    ...
    // Tattara tsarin data a cikin metadata
    const finalMetadata = {
    ...
    await prisma.auditLog
      .create({
        data: {
          ...
          description: `Created ${cleanName} (${normalizedTier}) at ${parsedSellingPrice} NGN`,
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to create service pricing.",
    });
  }
};
...
exports.updatePricing = async (req, res) => {
  try {
    ...
    if (req.body.serviceCode !== undefined) {
      data.serviceCode = normalizeUppercase(req.body.serviceCode);
    }
    ...
    if (req.body.sellingPrice !== undefined) {
      const sellingPrice = parseAmount(req.body.sellingPrice);
    ...
    const updatedMetadata = {
      ...
      ...(req.body.dataType ? { dataType: req.body.dataType } : {}),
      ...(req.body.dataSize ? { dataSize: req.body.dataSize } : {}),
      ...(req.body.validity ? { validity: req.body.validity } : {}),
    };
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to update service pricing.",
    });
  }
};

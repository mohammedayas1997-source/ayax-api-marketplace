...
const FRONTEND_URL = process.env.FRONTEND_URL || "https://www.ayaxapis.com";
...
const generateReference = (prefix = "AYAX-WALLET") => {
...
const createAuditLog = async ({ req, userId, userEmail, action, description }) => {
  try {
    ...
    console.error("Wallet audit log error:", error.message);
  }
};
  ...
  try {
    ...
    const [
      ...
      prisma.walletLedger.aggregate({
        where: { userId, type: { in: ["CREDIT", "REFUND", "REVERSAL"] } },
        _sum: { amount: true },
      }),
      prisma.walletLedger.aggregate({
        where: { userId, type: { in: ["DEBIT", "ADJUSTMENT"] } },
        _sum: { amount: true },
      }),
      prisma.walletLedger.aggregate({
        where: { userId, type: { in: ["CREDIT", "REFUND", "REVERSAL"] }, createdAt: { gte: todayStart } },
        _sum: { amount: true },
      }),
      prisma.walletLedger.aggregate({
        where: { userId, type: { in: ["DEBIT", "ADJUSTMENT"] }, createdAt: { gte: todayStart } },
        _sum: { amount: true },
      }),
      prisma.walletLedger.aggregate({
        where: { userId, type: { in: ["CREDIT", "REFUND", "REVERSAL"] }, createdAt: { gte: monthStart } },
        _sum: { amount: true },
      }),
      prisma.walletLedger.aggregate({
        where: { userId, type: { in: ["DEBIT", "ADJUSTMENT"] }, createdAt: { gte: monthStart } },
        _sum: { amount: true },
      }),
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        todayCredit: Number(todayCredit._sum.amount || 0),
        todayDebit: Number(todayDebit._sum.amount || 0),
        monthlyCredit: Number(monthlyCredit._sum.amount || 0),
        monthlyDebit: Number(monthlyDebit._sum.amount || 0),
        pendingFundings,
      },
      history: recentHistory.map(serializeLedger),
    });
  } catch (error) {
...
exports.getWalletTransactions = async (req, res) => {
  try {
    ...
    const { type, module, search, startDate, endDate } = req.query;
    const where = { userId };

    if (type) {
      const normalizedType = String(type).trim().toUpperCase();
      const allowedTypes = ["CREDIT", "DEBIT", "REFUND", "REVERSAL", "ADJUSTMENT"];

      if (!allowedTypes.includes(normalizedType)) {
        return res.status(400).json({
          success: false,
          message: "Invalid wallet transaction type.",
        });
      }
    ...
    if (search) {
      ...
      where.OR = [
        { reference: { contains: searchValue, mode: "insensitive" } },
    ...
    const [transactions, total, creditAggregate, debitAggregate] = await Promise.all([
      prisma.walletLedger.findMany({
        where,
        orderBy: { createdAt: "desc" },
        skip,
      ...
      prisma.walletLedger.aggregate({
        where: { ...where, type: { in: ["CREDIT", "REFUND", "REVERSAL"] } },
        _sum: { amount: true },
      }),
      prisma.walletLedger.aggregate({
        where: { ...where, type: { in: ["DEBIT", "ADJUSTMENT"] } },
        _sum: { amount: true },
      }),
    ]);
    ...
    return res.status(200).json({
      ...
      transactions: transactions.map(serializeLedger),
      history: transactions.map(serializeLedger),
      summary: {
        totalTransactions: total,
        totalCredit: Number(creditAggregate._sum.amount || 0),
        totalDebit: Number(debitAggregate._sum.amount || 0),
      },
      pagination: {
        page,
        limit,
...
exports.getMyTransactions = exports.getWalletTransactions;
...
exports.createFundingRequest = async (req, res) => {
  try {
    ...
    const channel = String(req.body.channel || "BANK_TRANSFER").trim().toUpperCase();
    ...
    const reference = generateReference("AYAX-FUND");
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to create funding request.",
    });
  }
};
...
exports.getMyFundingRequests = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      fundingRequests: fundingRequests.map(serializeFunding),
      requests: fundingRequests.map(serializeFunding),
      pagination: {
        page,
        limit,
        total,
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to retrieve funding requests.",
    });
  }
};

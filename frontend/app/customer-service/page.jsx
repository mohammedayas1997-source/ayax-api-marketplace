...
export default function CustomerServiceDashboard() {
  ...
  const fetchTickets = () => {
    setMessage("New support ticket received.");
  };

  const fetchRefundRequests = () => {
    setMessage("New refund request received.");
  };

  useEffect(() => {
    socket.connect();

    socket.on("support-ticket-created", fetchTickets);
    socket.on("refund-request-created", fetchRefundRequests);

    return () => {
      socket.off("support-ticket-created", fetchTickets);
      socket.off("refund-request-created", fetchRefundRequests);
      socket.disconnect();
    };
  }, []);
      ...
      prev.map((trx) =>
        trx.ref === ref
          ? {
              ...trx,
              status: "Processing",
              issue: "Retry sent for processing",
            }
          : trx
      )
    );
  ...
  return (
    <CustomerServiceLayout
      ...
      description="Search customers, inspect transactions, resolve issues, and send refund requests to Admin."
    >
      {message && (
      ...
      <section className="grid xl:grid-cols-[0.9fr_1.1fr] gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
            <h2 className="text-xl font-bold mb-5">Customer Transactions</h2>

...
const INITIAL_RESPONSE = {
  success: true,
  message:
    "Your live NIN verification response will appear here.",
  data: {
    reference: "AYAX-NIN-XXXXXXXX",
    status: "SUCCESSFUL",
  },
};
...
export default function NinDeveloperApiPage() {
      ...
      try {
        ...
        if (failed) {
          ...
          setMessage(
            getErrorMessage(
              failed.reason,
              "Some NIN API information could not be loaded."
            )
          );
        }
      } finally {
...
export async function verifyNin() {
  ...
  const testNinApi = async (
    ...
    if (!selectedPlan) {
      ...
      setMessage(
        "Select an active NIN pricing plan."
      );
    ...
    if (!consentAccepted) {
      ...
      setMessage(
        "Confirm that the NIN owner authorized this verification."
      );
    ...
    try {
      ...
      setMessage(
        response.data?.message ||
          "NIN API test completed successfully."
      );
  ...
  return (
    <DashboardLayout
      title="NIN Verification API"
      description="View pricing, test requests and integrate Ayax NIN verification in your application."
    >
      {message && (
        <div
      ...
      <section className="mb-8 rounded-3xl border border-slate-800 bg-slate-900 p-6">
        ...
        {pricingPlans.length === 0 ? (
          <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-5 text-yellow-300">
            No active NIN pricing plan is
            available. Super Admin must add
            NIN_VERIFY pricing first.
          </div>
        ) : (
        ...
        >
          <div className="mb-6">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-400">
              Test the NIN endpoint using
              your authenticated developer
              account.
            </p>
          </div>
          ...
          <div className="space-y-5">
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-800 bg-slate-950 p-4">
              ...
              <span className="text-sm leading-6 text-slate-400">
                I confirm that the NIN
                owner authorized this
                verification and the
                request is for a lawful
                purpose.
              </span>
            </label>
        ...
        <div className="mb-6 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              ...
              your backend server or a
              secured application service.
            </p>
          </div>
        </div>

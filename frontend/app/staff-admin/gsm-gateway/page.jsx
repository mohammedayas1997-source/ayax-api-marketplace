...
export default function GSMGatewayPage() {
  ...
  useEffect(() => {
    ...
    socket.on("gsm-low-balance", fetchGatewaySims);
    socket.on("gsm-balance-updated", fetchGatewaySims);

    return () => {
      socket.off("gsm-low-balance", fetchGatewaySims);
      socket.off("gsm-balance-updated", fetchGatewaySims);
      socket.disconnect();
    };
  }, []);
  ...
  const handleAutoSelectSim = () => {
    ...
    setMessage(
      `Auto selected ${bestSim.name} with ${formatNaira(
        bestSim.balance
      )} balance.`
    );
  };
  ...
  return (
    <StaffLayout
      title="GSM Gateway SIM Management"
      description="Monitor 32 GSM Gateway SIMs, balances, low balance alerts, and company recharge activity."
    >
      {message && (
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-2 leading-7">
              Staff can view company wallet balance and buy recharge plans for
              company SIMs only. Staff cannot withdraw company money and cannot
              change SIM numbers. SIM selection is handled automatically by the
              system.
            </p>
          </div>
        </div>
      ...
      <section className="grid xl:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
          <h2 className="text-xl font-bold mb-5">Buy Plan for Company SIM</h2>

          <label className="text-sm text-slate-300">Select Recharge Plan</label>
